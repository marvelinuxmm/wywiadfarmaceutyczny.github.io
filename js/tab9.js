/* Zakładka 9: Podsumowanie i raport — widok pełny / skrócony, druk, epikryza końcowa. */
(function () {
  const h = UI.h;
  ...
  function renderPelny(s) {
    ...
    /* 10. PC-MAI (matryca leków przeciwbólowych) */
    out.push(sekcja('10. PC-MAI — adekwatność farmakoterapii', [], pcmaBlock(s)));

    /* 11. Globalne flagi */
    out.push(sekcja('11. Alerty i flagi', [], flagaLista(G.Flags.compute(s))));

    return out;
  }

  /* Sekcja PC-MAI dla raportu: tabela leków z wynikiem MAI, suma i interpretacja. */
  function pcmaBlock(s) {
    const P = G.Pcmai;
    const p = s.pcmai || {};
    const wiersze = P.wybierzWiersze(s.leki, p.dodatkowe);
    if (!wiersze.length) {
      return h('p', { class: 'hint', text: 'Brak leków zakwalifikowanych do matrycy PC-MAI. Uzupełnij listę w zakładce „Farmakoterapia”.' });
    }
    let sumaTotal = 0;
    let nieprawidloweTotal = 0;
    const rows = wiersze.map(function (w) {
      const odp = (p.odpowiedzi || {})[w.id] || {};
      const wynik = P.policz(odp);
      sumaTotal += wynik.suma;
      nieprawidloweTotal += wynik.nieprawidlowe;
      const band = P.band(wynik.suma);
      const klasy = w.klasy.length ? ' [' + w.klasy.map(function (k) { return P.KLASA_LABEL[k] || k; }).join(', ') + ']' : '';
      return [w.nazwa + klasy,
        'oceniono ' + wynik.ocenione + '/' + wynik.liczba + ' kryteriów — ' + wynik.suma + '/' + wynik.max + ' pkt — ' + band.label];
    });
    const extra = h('div', {}, [
      h('div', { class: 'r-epikryza' }, [
        h('strong', { text: 'Suma PC-MAI (wszystkie leki): ' }),
        sumaTotal + ' / ' + (wiersze.length * P.MAX) + ' pkt; nieprawidłowe odpowiedzi: ' + nieprawidloweTotal + '.'
      ]),
      epikryzaBlock(p.epikryza, '')
    ]);
    return h('div', {}, [
      h('table', { class: 'raport-table' }, rows.map(function (r) {
        return h('tr', {}, [
          h('td', { class: 'r-key', text: r[0] }),
          h('td', { text: r[1] })
        ]);
  ...
  function renderSkrocony(s) {
    ...
    const pc = s.pcmai || {};
    const wierszePc = G.Pcmai.wybierzWiersze(s.leki, pc.dodatkowe);
    if (wierszePc.length) {
      let tot = 0;
      const opis = wierszePc.map(function (w) {
        const wyn = G.Pcmai.policz((pc.odpowiedzi || {})[w.id] || {});
        tot += wyn.suma;
        return w.nazwa + ' ' + wyn.suma + '/' + wyn.max;
      }).join('; ');
      wiersze.push(['PC-MAI — suma', tot + ' / ' + (wierszePc.length * G.Pcmai.MAX) + ' pkt (' + opis + ')']);
    }
    ...
    if (pc.epikryza && wierszePc.length) ep.push(['PC-MAI', pc.epikryza]);
    if (ep.length) {
  ...
  function build() {
    return [
      ...
      h('div', { class: 'card', style: { marginTop: '16px' } }, [
        h('h2', {}, [h('span', { class: 'num', text: '9' }), 'Epikryza końcowa']),
        h('div', { class: 'field' }, [
